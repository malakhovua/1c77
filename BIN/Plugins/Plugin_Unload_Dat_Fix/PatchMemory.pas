...
//            if LowerCase(Trim(DllName))=LowerCase(Trim(DllNameToFind)) then begin   // avgreen@molvest.org.ua
            if pos(LowerCase(Trim(DllNameToFind)), LowerCase(Trim(DllName))) = 1 then begin // Ищем не полное совпадение а совпадение начала строки. Причину смотри в модуле 'plugin_unload_dat_fix.dpr'
