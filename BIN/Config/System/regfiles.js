...
try {
	...
	var is64 = WshShell.ExpandEnvironmentStrings("%PROCESSOR_ARCHITECTURE%") != "x86";
	var regsvr32Dir = WshShell.ExpandEnvironmentStrings("%windir%") + (is64 ? "\\SysWOW64\\" : "\\system32\\");
...
function registerAll() {
	...
	if(testPreconditions())
    	...
    	//[+]Berdakov Dmitry, 14.02.2008, на чистом Win2003 нет этого OCX, поэтому неработают плагины написанные на VB (HistoryPlugin.dll,...)
    	var sysDir = fso.GetSpecialFolder(1).Path+"\\"; // \windows\system32\
	if (!fso.FileExists(sysDir+"MSCOMCTL.OCX")){
		fso.CopyFile(WshShell.CurrentDirectory+"\\MSCOMCTL.OCX", sysDir);
		regdll(sysDir+"MSCOMCTL.OCX");
	}

	sysDir = regsvr32Dir; // на всякий случай для \windows\SysWoW64\
	if (!fso.FileExists(sysDir+"MSCOMCTL.OCX")){
		fso.CopyFile(WshShell.CurrentDirectory+"\\MSCOMCTL.OCX", sysDir);
		regdll(sysDir+"MSCOMCTL.OCX");
	}
...
function regwsc(fullPath, params){
	return regsvr32Dir + "regsvr32.exe /s "+(params?params+" ":"")+'scrobj.dll /n /i:"'+fullPath+'"';
}
function regdll(fullPath, params){
	return regsvr32Dir + "regsvr32.exe /s " + (params?params+" ":"") + '"' + fullPath + '"';
}
