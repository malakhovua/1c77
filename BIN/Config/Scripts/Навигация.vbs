...
' к этому методу.
'
' Если под курсором находится выражение вида 'Спр.Нов|ый()' или просто то делается попытка определить место инициализации
...
Sub GoToObject()
	If IsTextWindow Then
		If TryToOpenObject("") Then Exit Sub  'Пытаемся открыть объект под курсором
		If TryVarDefJump() Then Exit Sub      'Переход к строке, где была определена переменная	
		If TryToOpenFile("") Then Exit Sub    'Пытаемся открыть внешний отчёт под курсором
		If TryOpenIncludeFile() Then Exit Sub 'Пытаемся Открыть файл из директивы ЗагрузитьИзФайла
		If TryClassMethodJump Then Exit Sub  'Переход в модуль класса
		
		If TryGotoTree() Then Exit Sub        'предполагается, что под курсором пустая строка - переходим к дереву ДМ
	Else
...
Sub ToggleWorkbookPage()
	...
	If Doc.Type <> docWorkBook Then Exit Sub
...
Function TryToOpenObject(WhatToOpen)
	...
	'message NameToOpen  TryToOpenObject = true : 
	If NameToOpen = "" Then Exit Function 'не нашли/не выбрали объекта
...
Sub SelectAndNavigateToClasses()
	...
	doc.ActivePage = 1
	Windows.ActiveWnd.Caption = Selection
