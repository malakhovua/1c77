...
/*===========================================================================
Copyright (c) 2004-200 Alexander Kuntashov
=============================================================================
...
function setMarker(markerType, newCode)
{
	...
	if (((doc.SelStartLine == doc.SelEndLine) && ((markerType != 2) || s.doNotCopyOldCode) && (!newCode))) {
		...
		var block		= doc.Range(doc.SelStartLine, 0, selEndLine, doc.LineLen(selEndLine)-1);
		...
		block += ind + "//" + AC_Marker + s.markerEndBlock + (s.doNotSignAtEnd?"":AC_Sign);
					        
		// вставляем сгенерированный код в документ	
		doc.Range(doc.SelStartLine, 0, selEndLine, doc.LineLen(selEndLine)-1) = block;		
...
function deleteLeftIndent(str,firstIndent) //[+]Berdakov Dmitry, 30.04.2007
{
	if (firstIndent)
		return str.replace(new RegExp("^"+firstIndent,"i"),"");
	return str;	
}
...
{
	var ret = "";
	var firstIndent = ind; //lines[0].match(/^(\s*)/)[0]; //[+]Berdakov Dmitry, 30.04.2007
	for (var i=0; i<lines.length; i++) {
		//ret += ind + "//" + lines[i] + "\r\n";
		ret += ind + "//" + deleteLeftIndent(lines[i],firstIndent) + "\r\n";
	}
	return ret;
}
