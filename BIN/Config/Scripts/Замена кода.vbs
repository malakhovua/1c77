...
Sub CopyLine()
	...
	if (doc.SelStartLine = doc.SelEndLine) then
		...
		'else
			line = doc.SelEndLine
			if doc.SelEndCol = 0 then : line = line - 1 : end if
			col = doc.LineLen(line)
    		doc.Range(doc.SelStartLine, 0, line, col) = doc.Range(doc.SelStartLine, 0, line, col)  & vbCrLf & doc.Range(doc.SelStartLine, 0, line, col)
		'end if
  	end if
