...
Sub RunEnterpise(bInShared)
  ...
  set wshShell = CommonScripts.WSH
	...
	CmdLine = RegReplaceText(CmdLine, "\s+/n[a-zA-Zа-яА-Я0-9_]+[\s|$]+"," /n" & AppProps(appUserName) & " ")
	'CmdLine = RegReplaceText(CmdLine, "\s+/p[a-zA-Zа-яА-Я0-9_]+[\s|$]+"," ")
	...
	if (iIBState = -1) and bInShared then '
		...
		strAnswer = "ВНИМАНИЕ: " & ob.IBStateToString(iIBState) & vbCrLf & vbCrLf & _
				"Вы хотите зайти в монопольный режим с переиндексацией ?"  & vbCrLf & vbCrLf & _
				"""Нет"" - Удалить файл 1SUSERS.DBF и зайти в разделенном" & vbCrLf & _
				"""Отмена"" - не запускать"
		answ = MsgBox(strAnswer, vbYesNoCancel, SelfScript.Name)
		if answ = vbYes then
			bNeedSharedMode = false
		elseif answ = vbNo then
			CommonScripts.FSO.DeleteFile IBDir & "1SUSERS.DBF", 0
		else	
			bNeedRun = false
		end if
	end if
