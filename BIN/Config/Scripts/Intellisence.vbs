...
Function RemoveComments()'[*]metaeditor, 26.09.2009   'Function RemoveComments(txtStrings)
	RemoveComments = ""
	MaxI = UBound(txtStrings) - 1
	For i = 0 To MaxI Step 1
		str = txtStrings(i)
		pos = InStr(str, "//")
		If pos > 0 Then if InStr(str, "//:") = 0 Then txtStrings(i) = Left(str, pos-1) '[*]metaeditor,26.09.2009 'If pos > 0 Then txtStrings(i) = Left(str, pos-1)
	Next
End Function

Private Function PrepareModuleText(Line, Col)
'ProfilerEnter("PrepareModuleText 1")
	txtStrings = split(doc.Range(0,0,Line), vbCrLf) '[*]metaeditor,26.09.2009, не ищем ниже текущей строки 'txtStrings = split(doc.text, vbCrLf)
'ProfilerLeave("PrepareModuleText 1")
'ProfilerEnter("PrepareModuleText 2")
	RemoveComments()'[*]metaeditor, 26.09.2009  'RemoveComments(txtStrings) иначе после вызова ф-ии исходный массив, переданный через параметр, остается неизменным, т.е с комментариями
...
Private Function PossibleTypes()
	...
	'[+]metaeditor, 25.09.2006, добавим в список все виды справочников и документов
	strMDTypes = ""
	Set childs = MetaData.TaskDef.childs
	For i = 0 To 2
		Set mda = childs(i)
		For j = mda.Count - 1 To 0 Step -1
			strMDTypes = strMDTypes & "," & childs.Name(i, True) & "." &  mda(j).Name
		Next
	Next
	'msgbox strMDTypes
	strPossibleTypes = strPossibleTypes + strMDTypes
	'[+]_
	...
	For Each f1 In fso.GetFolder(DataDir).Files
...
'	TryOnRightDefinition = GetMainType
	...
	'[+]metaeditor, 26.09.2009, т.к убран поиск ниже текущей строки, то надо поискать тип переменной в секции инициализации модуля
	if TryOnRightDefinition = "" then
		...
		QueryMeth.Pattern = "\s*(конецпроцедуры|конецфункции|endprocedure|endfunction)"
		QueryVar.Pattern = "\s*" & VarName & "\s*=\s*(создатьобъект|createobject|получитьпустоезначение)\(""([\wа-яё\d_.]+)""\);"
		for i = doc.LineCount-1 to 0 step -1
			str = doc.Range(i)
			if QueryMeth.Test(str) then Exit for
			Set Matches = QueryVar.Execute(str)
			if Matches.Count > 0 then
				TryOnRightDefinition = lCase(Matches(0).SubMatches(1))
				exit for
			end if
		next
	end if	
...
'ProfilerEnter("GetMainType 1")
	...
	'[+]metaeditor, 27.05.2008, иначе Навигация::TryVarDefJump прыгает не к тому номеру строки (без учета пустых строк)
	CrObjLine = "[\wа-яё]*" + "("+sRegVarNameSearch +"|"+ "перем"+ sRegVarNameSearch+")" + "(=[\wа-яё\d_.]*\.?(создатьобъект|перехватитьактивх|wrapactivex|createobject|получитьпустоезначение)\(""([\wа-яё\d_.]+)""\);*|[,;=]*//:([\wа-яё\d_.]+))"
...
Function GetExprType(aDoc, Expr)
	...
	'txtStrings = split(doc.text, vbCrLf) '[-]metaeditor
...
function Telepat_OnShowMemberList(Line, Col)
	message "Telepat_OnShowMemberList intel"
...
Sub ReadGlobalVariables()
	...
	for i = 0 to UBound(ModuleLines)
		...
		if QueryMeth.Test(ModuleLines(i)) then Exit For '[*]metaeditor, 26.09.2009
	...
	next
 	...
 	'[+]metaeditor, 26.09.2009, ищем типы гл. переменных в секции инициализации ГМ
	QueryMeth.Pattern = "\s*(конецпроцедуры|конецфункции|endprocedure|endfunction)"
	QueryVar.Pattern = "\s*([\wа-яё]+)\s*=\s*(создатьобъект|createobject|получитьпустоезначение)\(""([\wа-яё\d_.]+)""\);"
	for i = UBound(ModuleLines) to 0 step -1
		if QueryMeth.Test(ModuleLines(i)) then Exit Sub
		Set Matches = QueryVar.Execute(ModuleLines(i))
		if Matches.Count > 0  then
			if not GlobalVariables.Exists(UCase(Matches(0).SubMatches(0))) then
				GlobalVariables.Add UCase(Matches(0).SubMatches(0)),  LCase(Matches(0).SubMatches(2))
			end if
		end if
	next
