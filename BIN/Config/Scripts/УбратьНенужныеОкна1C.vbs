...
Sub Configurator_OnMsgBox(text, style, def, Answer)
	If text = "Реорганизация информации закончена!" Then
		Status UCase(Text)
		Answer = mbaOK
	End If	
	
	'If Instr(Text,"При сохранении метаданных будет произведен анализ сделанных  изменений и") = 1 then
	'	if not Scripts("CheckKeyState").IsShiftPressed then
	'		Status UCase(Left(Text,30))
	'		Answer = mbaYes
	'	end if
	'End if	
End Sub